...
const manufacturers_queried = new Map<string, Promise<any>>();
const manufacturer_stock = new Map<string, Map<string, string>>();

const refresh_manufacturer = async (manufacturer: string) => {
	try {
		const response = await superagent.get(API_URL + "availability/" + manufacturer)
		if(response.body.response == "[]") {
			// We assume that this is a bug that occasionally happens, and stock is actually available, and just use the last stock info retrieved
			return;
		}
...
const refresh_category = async (category: string) => {
 ...
 */
const refresh_data = async (interval: number) => {
